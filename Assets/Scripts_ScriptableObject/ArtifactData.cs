...
{
    [Header("기본 정보")] public string artifactName; // 아티팩트 이름
    [TextArea] public string description; // 설명
    public Sprite icon; // 아이콘
    public ArtifactGrade grade; // 등급
    public int basePrice = 100; // 상점 가격

    [Header("메커니즘 설정")] public ArtifactType type; // 작동 방식
    public ConditionType condition; // 발동 조건

    [Header("버프 설정 (Trigger 전용)")] public float buffDuration = 10f; // 버프 지속 시간 (기본 10초)
    public bool isInfiniteBuff = false; // 체크 시 무한 지속 (피격 등 특정 조건 전까지)

    [Header("첫 번째 효과")] public ArtifactEffectType effectType;
    public float value;
    public bool isPercent;

    [Header("두 번째 효과 (옵션)")] public ArtifactEffectType effectType2 = ArtifactEffectType.None;
    public float value2;

...
public class Dice_UI : MonoBehaviour
{
    ...
    private IEnumerator SingleRollRoutine(DiceData data, int diceIndex, Action onResultShown)
    {
        ...
        if (glowFilter != null)
            glowFilter.Color = data.uiGlowColor;
    ...
    private void SpawnBurstParticles(Vector3 centerPosition)
    {
        ...
        for (int i = 0; i < burstCount; i++)
        {
            ...
            pRect.localScale = Vector3.one * UnityEngine.Random.Range(0.3f, 0.5f);

            Vector2 randomDir = UnityEngine.Random.insideUnitCircle.normalized;
            float randomSpeed = UnityEngine.Random.Range(200f, 350f);
    ...
    private void UpdateDiceFill(bool isMax)
    {
        ...
        if (isMax)
        {
            ...
            if (diceSprites != null && diceSprites.Length >= 2)
                diceFillImage.sprite = diceSprites[1];
        }
        else
        {
            ...
            if (diceSprites != null && diceSprites.Length >= 2)
                diceFillImage.sprite = diceSprites[0];
        }
    }

...
{
    [Header("Ammo Visuals")]
    public Image[] bulletPanels; // 5개의 Fill 이미지
    public TextMeshProUGUI[] ammoText; // [0]:정수, [1]:소수, [2]:MAX 텍스트

    [Header("Ammo Sprites (New)")]
    [SerializeField] private Sprite normalAmmoSprite; // 평상시 하얀 총알 스프라이트
    [SerializeField] private Sprite maxAmmoSprite;    // ★ [추가] 가득 찼을 때의 주황색 전용 스프라이트
    ...
    [SerializeField] private Color emptyTextColor = Color.red;    // 0~99 (텍스트 전용)
    ...
    private void Awake()
    {
        ...
        // 시작할 때 기본 스프라이트 기억 (인스펙터에서 안 넣었을 경우 대비)
    ...
    private void UpdateAmmoVisuals()
    {
        ...
        // --- 2. 총알 이미지 및 스프라이트 교체 로직 ---
        // ★ [핵심] 탄약이 최대치일 때 쓸 스프라이트를 결정
        ...
        for (int i = 0; i < bulletPanels.Length; i++)
        {
            ...
            // ★ 스프라이트 직접 교체
            // image.color는 건드리지 않고, 이미지의 원본 소스(sprite)를 갈아끼웁니다.
    ...
    // ... (Co_ScaleAnimation, UpdateScrapVisuals, Co_ScrapPunch 기존 로직 동일) ...
